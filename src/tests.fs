...
25 constant BUZZER_PIN
0  constant AUDIO_CHANNEL
13 constant RESOLUTION  \ Résolution de 13 bits (standard)
5000 constant FREQ      \ Fréquence initiale (5kHz)


: init-audio ( -- )
  FREQ RESOLUTION AUDIO_CHANNEL ledcSetup drop
  BUZZER_PIN AUDIO_CHANNEL ledcAttachPin
;
...
: beep ( freq duration -- )
  BUZZER_PIN swap ledcWriteTone  \ Lance la fréquence
  ms                             \ Attend (durée en ms)
  BUZZER_PIN 0 ledcWriteTone     \ Coupe le son (fréquence 0)
;

: alerte
  1000 500 beep                  \ 1000 Hz pendant 500ms
  1500 500 beep                  \ 1500 Hz pendant 500ms
;
